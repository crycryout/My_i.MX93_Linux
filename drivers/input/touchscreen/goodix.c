...
static void goodix_ts_report_touch_8b(struct goodix_ts_data *ts, u8 *coor_data)
{
	...
	if (ts->goodix_xy_rotation90)
	    touchscreen_report_pos(ts->input_dev, &ts->prop,
				       input_y, ts->x_max - input_x, true);
	else
	    touchscreen_report_pos(ts->input_dev, &ts->prop,
...
static void goodix_read_config(struct goodix_ts_data *ts)
{
	...
	if (x_max && y_max) {
		if (ts->goodix_xy_rotation90) {
			input_abs_set_max(ts->input_dev, ABS_MT_POSITION_X, y_max - 1);
			input_abs_set_max(ts->input_dev, ABS_MT_POSITION_Y, x_max - 1);
		} else {
...
static int goodix_configure_dev(struct goodix_ts_data *ts)
{
	...
	if (device_property_read_bool(ts->input_dev->dev.parent, "touchscreen-xy-rotation90"))
        ts->goodix_xy_rotation90 = !ts->goodix_xy_rotation90;
