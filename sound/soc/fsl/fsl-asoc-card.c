...
struct fsl_asoc_card_priv {
	...
	struct asoc_simple_jack spk_jack;
...
struct fsl_asoc_card_jack {
    struct asoc_simple_jack spk_jack;
};

struct fsl_asoc_card_jack alientek_alsa_jack;
			 ...
			 void *data)
{
	...
	if (event & SND_JACK_HEADPHONE) {
		...
		gpiod_set_value(alientek_alsa_jack.spk_jack.gpio.desc, 0);
		return snd_soc_dapm_disable_pin(dapm, "Ext Spk");
	}
	else {
		gpiod_set_value(alientek_alsa_jack.spk_jack.gpio.desc, 1);
...
static int spk_jack_event(struct notifier_block *nb, unsigned long event,
			  void *data)
{
	return 0;
}

static struct notifier_block spk_jack_nb = {
	.notifier_call = spk_jack_event,
};
...
static int fsl_asoc_card_probe(struct platform_device *pdev)
{
    ...
    if (of_property_read_bool(np, "spk-det-gpio")) {    /* add it */
        ret = asoc_simple_init_jack(&priv->card, &priv->spk_jack,
						2, NULL, "Spk Jack");
        alientek_alsa_jack.spk_jack = priv->spk_jack; 
        if (ret)
            goto asrc_fail;     
	
		snd_soc_jack_notifier_register(&priv->spk_jack.jack, &spk_jack_nb);                    
    }
asrc_fail:
	of_node_put(asrc_np);
